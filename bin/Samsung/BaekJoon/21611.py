...
def check_explode():
    ...
    for x,y in numbers[2:]:
        if temp != mat[x][y]: # 이전값이랑 다음 값비교하는것
            cnt = 1
            temp_arr = [(x,y)]
        elif temp == mat[x][y] and temp != 0:
            cnt += 1
            temp_arr.append((x,y)) # temp_arr
        if cnt >= 4:
            arr.extend(temp_arr)
            temp_arr = []
        temp = mat[x][y]
...
def move(mat):  # 빈칸을 채우기 위해 구슬이 이동하는 함수
    move_pos = deque()
    for x,y in numbers:
        if x == N//2 and y == N//2:
            continue
...
def grouping(mat):  # 그룹핑 하는 함수
    ...
    for x, y in numbers: # 1번칸이면 (3,2)
        if x == N // 2 and y == N // 2:
            continue
        if num == -1:
            num = mat[x][y] # 구슬의 정보를 담고 있는 배열
            count = 1
        else:
            if num == mat[x][y]:
                count += 1
            else:
                nums.append(count) # 개수
                nums.append(num) # 번호
                num = mat[x][y] # 구슬의 번호
                count = 1
